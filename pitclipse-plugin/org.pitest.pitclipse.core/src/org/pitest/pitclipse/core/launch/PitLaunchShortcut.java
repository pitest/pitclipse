...
public class PitLaunchShortcut extends JavaLaunchShortcut {
	...
	private static final class ShortcutCreationException extends
			RuntimeException {
	...
	protected ILaunchConfiguration createConfiguration(IType type) {
		...
		try {
			wc = configType.newInstance(null, launchManager
					.generateLaunchConfigurationName(type
			...
			wc.setAttribute(
					...
					projectName);
			log("Creating new configuration for project: " + projectName
	...
	/**
	 * Returns the Java elements corresponding to the given objects. Members are
	 * translated to corresponding declaring types where possible.
	 * 
	 * @param objects
	 *            selected objects
	...
	private List<IType> getJavaElements(Object[] objects) {
		...
		for (Object object : objects) {
			if (object instanceof IAdaptable) {
				Object element = ((IAdaptable) object)
	...
	protected IType[] findTypes(Object[] elements, IRunnableContext context)
		...
		// try {
		/*
		 * if(elements.length == 1) { IType type = isMainMethod(elements[0]);
		 * if(type != null) { return new IType[] {type}; } }
